...
def explain_anomaly(anomaly_score: float, feature_contributions: dict[str, float]) -> dict[str, Any]:
    ...
    return {
        ...
        "confidence": 0.85,
        "recommended_investigation": f"Investigate recent changes in {sorted_features[0][0]} if it is a leading indicator." if sorted_features else "No clear feature driver found."
    }
