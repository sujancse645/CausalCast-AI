...
def main():
    ...
    joblib.dump(model, r"models\online_retail\lightgbm_model.pkl")
    
    with open(r"reports\online_retail\lightgbm_metrics.json", 'w') as f:
        json.dump(metrics, f, indent=4)
        
    df_pred = df_test[['Date', 'Revenue']].copy()
    df_pred['prediction'] = y_pred
    df_pred.to_parquet(r"reports\online_retail\lightgbm_predictions.parquet", index=False)
    
    logger.info("LightGBM pipeline complete.")
