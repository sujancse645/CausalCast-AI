...
class Explanation(Base):
    ...
    __table_args__ = (
        Index("ix_explanation_model_type", "model_run_id", "explanation_type"),
    )
    
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    explanation_type: Mapped[str] = mapped_column(String(50), index=True) # global_feature_importance, local_feature_attribution, etc.
...
class DiagnosticReport(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    report_type: Mapped[str] = mapped_column(String(50), index=True) # residual_analysis, anomaly, drift, model_disagreement
    model_run_id: Mapped[str | None] = mapped_column(ForeignKey("forecast_model_runs.id", ondelete="CASCADE"), nullable=True)
