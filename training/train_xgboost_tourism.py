...
def main():
    ...
    joblib.dump(model, r"models\tourism\xgboost_model.pkl")
    
    with open(r"reports\tourism\xgboost_metrics.json", 'w') as f:
        json.dump(metrics, f, indent=4)
        
    df_pred = df_test[['series_name', 'time_index', 'value']].copy()
    df_pred['prediction'] = y_pred
    df_pred.to_parquet(r"reports\tourism\xgboost_predictions.parquet", index=False)
