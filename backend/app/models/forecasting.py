...
class ForecastExperiment(Base):
    ...
    experiment_family: Mapped[str] = mapped_column(String(40), default="classical")
    deep_forecasting_enabled: Mapped[bool] = mapped_column(Boolean, default=False)
    deep_engine: Mapped[str | None] = mapped_column(String(40), nullable=True)
    deep_data_config_json: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)
    deep_runtime_config_json: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)
    deep_readiness_status: Mapped[str | None] = mapped_column(String(40), nullable=True)
...
class ForecastModelRun(Base):
    ...
    model_framework: Mapped[str | None] = mapped_column(String(50), nullable=True)
    model_architecture: Mapped[str | None] = mapped_column(String(80), nullable=True)
    accelerator: Mapped[str | None] = mapped_column(String(20), nullable=True)
    ...
    training_framework_version: Mapped[str | None] = mapped_column(String(50), nullable=True)
    torch_version: Mapped[str | None] = mapped_column(String(50), nullable=True)
    cuda_version: Mapped[str | None] = mapped_column(String(50), nullable=True)
    deep_model_status: Mapped[str | None] = mapped_column(String(40), nullable=True)
...
class DeepForecastCheckpoint(Base):
    __tablename__ = "deep_forecast_checkpoints"
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    model_run_id: Mapped[str] = mapped_column(ForeignKey("forecast_model_runs.id", ondelete="CASCADE"), index=True)
    checkpoint_type: Mapped[str] = mapped_column(String(30))
    epoch: Mapped[int | None] = mapped_column(Integer, nullable=True)
    global_step: Mapped[int | None] = mapped_column(Integer, nullable=True)
    monitored_metric: Mapped[str | None] = mapped_column(String(50), nullable=True)
    monitored_value: Mapped[float | None] = mapped_column(Float, nullable=True)
...
class DeepForecastReadinessSnapshot(Base):
    __tablename__ = "deep_forecast_readiness_snapshots"
    __table_args__ = (Index("ix_deep_readiness_prepared_created", "prepared_dataset_id", "created_at"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    prepared_dataset_id: Mapped[str] = mapped_column(ForeignKey("prepared_datasets.id", ondelete="CASCADE"), index=True)
    readiness_status: Mapped[str] = mapped_column(String(40), index=True)
    engine: Mapped[str] = mapped_column(String(40))
    model_name: Mapped[str] = mapped_column(String(60))
    target_column: Mapped[str] = mapped_column(String(255))
    time_column: Mapped[str] = mapped_column(String(255))
    group_columns_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    horizon: Mapped[int] = mapped_column(Integer)
    input_size: Mapped[int] = mapped_column(Integer)
    series_count: Mapped[int] = mapped_column(Integer)
    eligible_series_count: Mapped[int] = mapped_column(Integer)
    historical_covariates_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    future_covariates_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    static_covariates_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    ...
    artifact_checksums_json: Mapped[dict[str, str]] = mapped_column(JSON, default=dict)
    checksum: Mapped[str] = mapped_column(String(64))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
